...
<div class="nav-tabs-custom">
    <ul class="nav nav-tabs">
        ...
        <li class=""><a href="#warehouses" data-toggle="tab" aria-expanded="false">Warehouses</a></li>
                    ...
                    <table class="table table-striped">
                        <thead>
                            <tr>
                                ...
                                <th scope="col" class="text-center">First Entry Date</th>
                                <th scope="col" class="text-center">Last Update</th>
                                <th scope="col" class="text-center">Current Status</th>
                            </tr>
                        </thead>
                        <tbody>
                            @if(isset($resource->inventories) && !$resource->inventories->isEmpty())
                                @foreach($resource->inventories as $key => $inventory)
                                    <tr>
                                        <th scope="row">
                                            {{ $inventory->warehouse->name }}
                                            @switch($inventory->warehouse->status)
                                                @case('disabled')
                                                    <span class="badge badge-pill label label-danger">Disabled</span>
                                                @break
                                            @endswitch
                                        </th>
                                        <td class="text-center">{{ $inventory->quantity }}</td>
                                        <td class="text-center">{{ date('F d, Y', strtotime($inventory->created_at)) }}</td>
                                        <td class="text-center">{{ date('F d, Y', strtotime($inventory->updated_at)) }}</td>
                                        <td class="text-center">
                                            @switch($inventory->quantity)
                                                @case(0)
                                                    <span class="badge badge-pill label label-secondary">Out of Stock</span>
                                                @break
                                                @default
                                                    <span class="badge badge-pill label label-success">In Stock</span>
                                                @break
                                            @endswitch
                                        </td>
                                    </tr>
                                ...
                                <tfooter>
                                    ...
                                    <td colspan="2" class="bg-primary">
                                        @switch($resource->inventories->sum('quantity'))
                                           @case(0)
                                            <h5 style="margin:0">
                                               <span class="label label-danger" style="padding-left: 0"> 
                                                    <span class="badge label-primary" style="border-radius: 0; margin-right:5px;"> 
                                                        {{ $resource->inventories->sum('quantity') }} 
                                                    </span> Out of Stock
                                                </span>
                                            </h5>
                                           @break
                                           @default
                                                <h5 style="margin:0">
                                                   <span class="label label-success" style="padding-left: 0"> 
                                                        <span class="badge label-primary" style="border-radius: 0; margin-right:5px;"> 
                                                            {{ $resource->inventories->sum('quantity') }} 
                                                        </span> In Stock
                                                    </span>
                                    ...
                                    <th class="bg-primary text-center">Availble Quantity: </th>
                                                ...
                                                <h5 style="margin:0">
                                                   <span class="label label-success" style="padding-left: 0"> 
                                                        <span class="badge label-primary" style="border-radius: 0; margin-right:5px;"> 
                                                            @php
                                                                $available_stock = 0;
                                                                if($resource->inventories && !$resource->inventories->isEmpty()){
                                                                    foreach ($resource->inventories as $key => $inventory) {
                                                                        if(isset($inventory->warehouse) && $inventory->warehouse->status == 'active'){
                            ...
                            @else
                                <th colspan="5">No Inventories</th>
                            @endif
                        </tbody>
                    </table>
                </div>
