...
//    	dd($this->lang);
    ...
    public function index($slug = null)
    {
        ...
        if($pageObject){
            ...
            // IF page has child page(s), redirect to first child page.
            if(!$submenu->isEmpty()){
            	$pageObject = $this->page->where('slug', $submenu[0]->slug)->whereStatus('published')->first();
            	if($pageObject && $pageObject->permalink){
            	    return redirect()->to($pageObject->permalink);
                }
            }
            ...
            // dd($submenu, $siblingmenu, $parentmenu);
