<tr data-id="{{ $category->id }}" data-order="{!! $category->order !!}" data-position="{!! $key !!}">
	...
	<td class="action">
		...
		<a href="{{ route('categories.edit', [$category->id, $category->type]) }}"   class="btn btn-xs btn-warning"><i class="fa fa-edit"></i> Edit</a>
		<a href="{{ route('categories.destroy', $category->id ) }}" title="Delete" class="btn btn-xs btn-danger confirm-del"><i class="fa fa-trash"></i> Remove</a>
	</td>
</tr>
