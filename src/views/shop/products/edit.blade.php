	...
	<div class="box">
	    <div class="box-header with-border">
	        ...
	        @else
				...
				<a href='{{ url("admin/$module/create") }}' class="btn btn-primary btn-flat pull-right ">Add New</a>
			@endif
	    </div>
	    <div class="box-body">
	        {{-- @include('admin-panel::resource.parts.forms._create_edit_form') --}}
	        @include('admin-panel::shop.products.parts.forms._create_edit_form')
	    </div>
	    <!-- /.box-body -->
	</div>
...
@section('script')
	...
	{!! JsValidator::formRequest('\Codeman\Admin\Http\Requests\Shop\ProductRequest') !!}
	<script>
        $('select').select2();
	  	$(function () {
	  		if($('.editor').length > 0){
	...
	<script>
	 	$('#attributes').off('click','#option_groups_add_btn').on('click', '#option_groups_add_btn', function (e) {
	 		...
	 		$.ajax({
	 		    type: 'GET',
	 		    url: app.ajax_url + '/admin/product-options/'+selected_group_id+'/options',
	 		    dataType: 'JSON',
	 	...
	 	$('#variations').off('click','#generate_variations_button').on('click', '#generate_variations_button', function (e) {
	 		let val = $('#generate_variations_option').val();
	 		let productID = $(this).data('product-id');
	 		console.log(val, productID);
	 		$.ajax({
	 		    type: 'GET',
	 		    url: app.ajax_url + '/admin/variations-generate/'+productID+'/'+val,
	 		    dataType: 'JSON',
	 		    success: function(data){
	 		    	console.log(data);
	 		        if(data.status == 'success'){
	 		        	$('#variations').find('.options-container').append(data.html);
	 		        	// $('#accordion-v-'+selected_group_id).find('.select2').select2();
	 		        }
	 		    }
	 		});
	 	});
